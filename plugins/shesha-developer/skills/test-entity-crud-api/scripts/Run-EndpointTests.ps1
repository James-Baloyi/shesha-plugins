...
# Usage: .\Run-EndpointTests.ps1 [-StartServer] [-UpdateEntities] [-FullErrors] [-Port 5000]
# ============================================================================
...
# Determine repository root - navigate up from .claude/skills/test-entity-crud-api/scripts
if ($RepoRoot -eq "") {
...
function Start-BackendServer {
    ...
    # Start server in background (use --no-build since we already built)
    $process = Start-Process -FilePath "dotnet" `
        -ArgumentList "run", "--project", $script:WebHostProject, "--no-build" `
        -WorkingDirectory $BackendDir `
        -PassThru `
        -WindowStyle Hidden
